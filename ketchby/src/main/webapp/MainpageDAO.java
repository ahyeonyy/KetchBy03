...
public class MainpageDAO {
	...
	public ArrayList<HotClubVO> findHotClub(){
		...
		String sql="select * "
				+ "from ( "
				+ "    select cb.cb_no, cb.cb_title, cb.cb_people, count(ca.cb_ap_no) AS ap_cnt,  "
				+ "			cb.cb_img, bc.bca_name, sc.sca_name , TO_CHAR(cb.cb_cls_date, 'MM/DD')  "
				+ "    from club cb  "
				...
				+ "    group by cb.cb_no, cb.cb_title, cb.cb_people, cb.cb_img, bc.bca_name, sc.sca_name, cb_cls_date  "
				+ "    order by count(ca.cb_ap_no) / cb.cb_people desc "
				+ ") "
				+ "where rownum <=3";
		...
		public ArrayList<HotBoardVO> findHotBoard(){
			...
			String sql="select * "
					+ "from ( "
					+ "  select b.b_no, b.b_title, b.b_content, TO_CHAR(b.b_date, 'MM/DD'), a.a_no, a.a_nick, a.a_img, bc.bc_name "
